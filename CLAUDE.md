...
### 9. 언어 설정은 화면 문구와 AI 분석 결과에 함께 적용한다
설정 화면의 언어 전환(`setLanguage()`, `js/script.js`의 `I18N` 객체)은 한국어·중국어·베트남어·태국어·우즈베크어를 지원한다. 문서·문자 분석과 후속 질문 요청에는 현재 `appState.settings.language`를 전송하고, Worker는 JSON 키와 status 값은 유지하면서 사용자가 읽는 문장을 선택 언어로 생성하도록 지시한다. 날짜·금액·전화번호·URL과 고유명사는 원문을 유지하며, 결과 음성 안내도 같은 언어의 TTS 코드를 사용한다. Thai/Uzbek 번역은 실제 사용 전 원어민 검수를 권장한다.
