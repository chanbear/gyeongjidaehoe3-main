...
> 참고: 안드로이드 패키지명(`com.ondam.app`)과 표시 이름은 "온담"으로 바뀌었고, Worker는 `ondam-ai.kke88084.workers.dev`로 재배포되어 있다. 다만 D1 데이터베이스명(`ansim-doumi-db`)·GitHub 저장소명 등 나머지 식별자는 재배포 부담을 피하기 위해 이전 이름("안심도우미"/`ansimhelper`/`an
...
`worker/src/index.js`는 `/analyze-doc`, `/analyze-text` 엔드포인트로 이미지·문자를 받아 Anthropic API(`claude-opus-4-8`, `json_schema` 출력)로 분석하고 `{status, headline, summary, checklist}`를 반환하며, `https://ondam-ai.kke88084.workers.dev`로 배포되어 있다. `js/script.js` 상단의 `AI_WORKER_URL` 상수가 이 주소를 가리키고, `analyzeDocument()`/`analyzeSmsText()`가 사진 촬영·붙여넣기 직후 이 엔드포인트를 호출한다. Worker 코드를 고치면 `worker/` 안에서 `npm run deploy`로 다시 배포해야 실사용 흐름에 반영된다.
...
**회원가입/로그인:** 최초 실행 시 `screen-greet`의 "시작하기" → `screen-signup`(이름/전화번호/4자리 PIN/PIN 확인) → Worker `POST /signup`(PIN은 salt+SHA-256 해시로만 저장, D1 `users` 테이블) → 성공하면 발급된 토큰을 `localStorage`의 `ai_helper_auth_v1`(`AUTH_KEY`)에 저장하고 `screen-profile`(성별/연령대/지역, 선택) → `screen-guardian-profile`(보호자 정보, 선택) → `startCoachmark()` 튜토리얼로 이어진다. 이미 계정이 있으면 `screen-login`(전화번호+PIN) → Worker `POST /login`으로 토큰 발급. 5회 연속 PIN 실패 시 15분 잠금(423)이며, 로그인 실패(401 부적절)와 잠금(423)을 구분해 응답하는 것은 "계정이 존재하는지"를 노출하는 트레이드오프임을 의도적으로 감수한 것(설계 문서 `docs/superpowers/specs/2026-07-30-account-signup-design.md` 참고, 경진대회 데모 범위의 트레이드오프). PIN을 잊었을 때는 `screen-login`의 "비밀번호를 잊으셨나요?" → `screen-reset-pin`에서 이름+전화번호 입력 → `POST /request-pin-reset-otp`가 알리고(Aligo)로 6자리 OTP를 문자 발송(`otp_hash`/`otp_expires_at`/`otp_attempts` 컬럼 사용, 5분 만료·5회 실패 시 잠금) → `POST /verify-pin-reset-otp`로 OTP 검증 후 새 PIN 저장. **`ALIGO_API_KEY`/`ALIGO_USER_ID`/`ALIGO_SENDER` 3종 시크릿이 실제로 설정되어 있어야 문자가 실제로 발송된다** — 하나라도 없으면 `sendAligoSms()`가 조용히 `false`를 반환하고 `/request-pin-reset-otp`는 502 `sms_failed`를 응답한다(OTP 자체는 DB에 저장됨). 자격 증명 미설정 상태에서는 이 복구 흐름이 막힌다는 점을 데모 전에 확인할 것.
