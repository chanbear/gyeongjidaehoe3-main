...
### 6. 전화번호 로그인이 필수이며, `appState` 전체가 계정에 묶여 기기 간 동기화된다
2026-07-30 개편으로 이 앱은 더 이상 "로그인 없음" 구조가 아니다. 로그인해야만 실사용 화면(홈 등)에 들어갈 수 있다. 같은 날 두 번째 개편으로 비밀번호(PIN) 입력 자체를 없애고 **전화번호만으로 가입/로그인**하게 바뀌었다(8번 항목 참고 — 전화번호를 아는 사람이면 누구나 그 계정에 들어갈 수 있다는 트레이드오프를 의도적으로 감수한 경진대회 데모 범위의 선택). `appState`(기록·일정·설정·프로필·보호자 정보 전체)는 여전히 `saveState()`/`loadState()`로 `localStorage`(`ai_helper_state_v1` 키)에 저장되지만, 로그인된 상태에서는 `queueStateSync()`가 매 저장 시점마다 이 JSON 전체를 Worker `POST /state`로도 보내 Cloudflare D1에 백업한다(자세한 내용은 8번 항목).
...
### 8. 회원가입(전화번호만)과 `appState` 전체 서버 동기화 — `deviceId`/`profiles` 방식은 제거됨
...
**회원가입/로그인 (2026-07-30, 같은 날 두 번째 개편으로 비밀번호 완전 제거):** 처음에는 전화번호+PIN(비밀번호) 방식으로 만들었지만, 곧바로 **PIN을 완전히 없애고 전화번호만으로 가입/로그인**하도록 다시 바꿨다 — **전화번호를 아는 사람이면 누구나 그 계정으로 로그인해 기록·일정·보호자 정보를 볼 수 있다는 보안 트레이드오프를 사용자가 명시적으로 승인**했다(경진대회 데모 범위에서 로그인 마찰을 최소화하기 위한 의도적 선택). 최초 실행 시 `screen-greet`의 "시작하기" → `screen-signup`(이름/전화번호만) → Worker `POST /signup`(D1 `users` 테이블에 전화번호+토큰 저장, `pin_hash`/`pin_salt` 컬럼은 스키마 마이그레이션을 피하려 빈 문자열로만 채우고 더는 검증하지 않는다) → 성공하면 발급된 토큰을 `localStorage`의 `ai_helper_auth_v1`(`AUTH_KEY`)에 저장하고 `screen-profile`(성별/연령대/지역, 선택) → `screen-guardian-profile`(보호자 정보, 선택) → 이어진다. 이미 계정이 있으면 `screen-login`(전화번호만) → Worker `POST /login`이 해당 전화번호로 가입된 계정이 있는지만 확인해 토큰을 발급한다(없으면 404 `not_found`, 있으면 무조건 200). PIN 재설정 화면(`screen-reset-pin`)과 관련 Worker 엔드포인트(`/request-pin-reset-otp`, `/verify-pin-reset-otp`)·알리고(Aligo) SMS 연동은 PIN 자체가 없으므로 함께 삭제됐다 — 더 이상 이 프로젝트에 알리고 관련 시크릿·의존성이 없다.
...
설정 화면의 언어 전환(`setLanguage()`, `js/script.js`의 `I18N` 객체)은 경기도 거주 외국인주민 통계에서 비중이 높은 4개 언어(중국어·베트남어·태국어·우즈베크어)를 지원한다 — 근거는 행정안전부·경기도여성가족재단 등록외국인 통계이며 정확한 순위는 자료마다 다소 차이가 있어 참고용이다. `data-i18n` 속성이 붙은 요소(홈 화면 인사말·카드·설정 화면 섹션 제목 등 핵심 UI)만 번역되고, `applyLanguage()`가 `I18N[lang]`으로 `innerHTML`을 교체한다. **이 문서의 옛 설명과 달리, 번역은 더 이상 사전에 다 박아둔 정적 5개 언어 블록만으로 채워지지 않는다** — `zh`/`vi`/`th`/`uz` 블록에는 기존 화면들의 번역이 정적으로 남아있지만, 계정/문자자동읽기 개편 이후 새로 추가되는 화면 문구는 `I18N.ko`에만 등록하고 `translateUiIfNeeded(lang)`이 필요할 때 Worker `POST /translate`(Claude API)로 그 언어 전체를 실시간 번역해 `localStorage`(번역 캐시 키)에 저장해두는 방식을 쓴다 — 정적 블록과 API 캐시 두 메커니즘이 공존한다. **AI가 생성하는 문서/문자 분석 결과(headline/summary/checklist)는 번역하지 않고 항상 한국어로 유지**한다 — 원문 오역으로 인한 안전 문제를 피하기 위함(공공데이터 원문을 한국어로 유지하는 다른 프로젝트의 관행과 동일한 이유). 새 언어를 추가하거나 문구를 늘릴 때 이 원칙을 유지할 것 — Thai/Uzbek 정적 번역은 초벌 번역이라 실제 사용 전 원어민 검수를 권장.
