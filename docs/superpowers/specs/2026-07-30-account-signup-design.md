...
**⚠️ 알려진 한계(의도적으로 받아들인 트레이드오프)**: 존재하지 않는 전화번호는 항상 `401 invalid`를
받지만, 실제 계정이 5회 이상 틀려 잠긴 상태면 `423 locked`을 받는다 — 이 차이 자체가 "그 전화번호로 가입된
계정이 있다"는 걸 공격자에게 알려주는 통로가 된다(로그인 화면에 "너무 여러 번 틀렸어요" 잠금 안내를 보여줘야
한다는 요구사항과 "계정 존재 여부를 노출하지 않는다"는 요구사항이 서로 부딪히는 지점). 같은 이유로 존재하지
않는 계정 응답은 PIN 해시 계산·DB 쓰기를 건너뛰어 잠긴 계정보다 근소하게 빠르게 응답하는 타이밍 차이도 있다.
이 앱은 금융·개인정보가 아니라 일정·기록 정도만 다루는 경진대회용 데모이고, PIN 재설정도 실제 본인확인
없이 이름 일치만으로 통과시키는 등 이미 비슷한 수준의 트레이드오프를 택하고 있어(위 재설정 문단 참고),
이 정도 계정 존재 노출은 고치지 않고 그대로 두기로 확정했다. 나중에 실서비스로 확장한다면 이 부분부터
다시 봐야 한다.
