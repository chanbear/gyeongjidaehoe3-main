...
/** APK(안드로이드)에서 Capacitor가 네이티브 플러그인 브릿지(window.Capacitor)를 심어주는 대상은
 *  웹뷰가 최초로 탐색(navigate)한 최상위 문서뿐이다 — <iframe>으로 별도 HTML(easy/app.html)을
 *  불러오면 그 문서는 별도의 window라 자체적으로는 window.Capacitor가 없다. 그 결과 쉬운 모드에서
 *  문자 확인(SmsReader)·주변 복지센터 찾기(Geolocation) 같은 네이티브 기능이 전부 "미지원"으로
 *  처리되어 동작하지 않는 문제가 있었다. 같은 오리진이라 이 window 객체를 그대로 참조로 넘겨줄 수
 *  있으므로, 부모가 이미 갖고 있는 window.Capacitor를 iframe의 window에 그대로 심어준다
 *  (플러그인 함수들은 원래 클로저(부모 window)를 그대로 쓰므로 참조만 넘겨도 정상 동작한다). */
function syncEasyCapacitorBridge(frame){
  const win = frame.contentWindow;
  if (!win || !window.Capacitor) return;
  if (!win.Capacitor) win.Capacitor = window.Capacitor;
}
...
function toggleEasyMode(checkbox){
  ...
  if (!frame.getAttribute('src')) {
    frame.addEventListener('load', () => {
      syncEasySafeArea(frame);
      syncEasyCapacitorBridge(frame);
    }, { once: true });
...
const I18N = {
  ...
  zh: {
    ...
    'error.aiVoice': '现在暂时无法分析。请稍后再试。',
      ...
      'deleteAccount.confirm': '确定退出',
      'deleteAccount.desc': '记录、日程、设置等已保存的所有信息将立即删除，且无法恢复。',
      'deleteAccount.errorGeneric': '处理失败了。请稍后再试。',
      'deleteAccount.errorPin': '密码不正确。',
      'deleteAccount.pinLabel': '请输入密码以确认身份',
      'deleteAccount.title': '真的要退出账户吗？',
      'onboard.guardian.desc': '收到危险短信时可以立即通知子女，<br>使用紧急求助按钮拨打电话时也会用到。',
      'onboard.guardian.title': '可以告诉我们<br>子女(监护人)的信息吗？',
      'onboard.guardian.voice': '有急事时要通知的子女或监护人的姓名和电话号码，您愿意告诉我们吗？如果不需要，可以跳过。',
      'onboard.login.errorInvalid': '电话号码或密码不正确',
      'onboard.login.errorLocked': '错误次数过多。请15分钟后再试',
      'onboard.login.forgotPin': '忘记密码了吗？',
      'onboard.resetPin.desc': '请输入注册时使用的姓名和电话号码，我们会通过短信发送验证码。',
      'onboard.resetPin.errorOtpExpired': '验证码已过期，请重新获取',
      'onboard.resetPin.errorOtpInvalid': '验证码不正确（还剩{n}次）',
      'onboard.resetPin.errorOtpLocked': '错误次数过多，请从头重新开始',
      'onboard.resetPin.errorSmsFailed': '短信发送失败，请稍后再试',
      'onboard.resetPin.otpLabel': '验证码（6位数）',
      'onboard.resetPin.otpSentNotice': '验证码已发送',
      'onboard.resetPin.requestOtp': '获取验证码',
      'onboard.resetPin.submit': '重新设置',
      'onboard.resetPin.successNotice': '密码已重新设置。请使用新密码登录。',
      'onboard.resetPin.title': '重设密码',
      'onboard.resetPin.voice': '输入姓名和电话号码后，我们会通过短信发送验证码。',
      'onboard.signup.backHome': '← 返回主页',
      'onboard.signup.desc': '请输入电话号码和密码。<br>没有账户的话会自动为您创建新账户。',
      'onboard.signup.errorGeneric': '处理失败了，请稍后再试',
      'onboard.signup.errorPhone': '请重新确认电话号码',
      'onboard.signup.errorPinFormat': '密码请输入4位数字',
      'onboard.signup.errorPinMismatch': '您输入的两次密码不一致',
      'onboard.signup.google': 'Google 登录',
      'onboard.signup.kakao': 'Kakao 登录',
      'onboard.signup.naver': 'Naver 登录',
      'onboard.signup.or': '或',
      'onboard.signup.phoneLabel': '电话号码',
      'onboard.signup.pinConfirmPlaceholder': '请再次输入密码',
      'onboard.signup.pinLabel': '密码',
      'onboard.signup.pinPlaceholder': '请输入密码',
      'onboard.signup.submit': '登录·注册',
      'onboard.signup.title': '登录或<br>注册账户',
      'onboard.signup.voice': '请输入电话号码和密码。没有账户的话会自动为您创建新账户。',
      'privacy.deleteDesc': '在设置画面点击"退出账户"，账户及其保存的所有信息将立即从服务器删除。',
      'privacy.deleteTitle': '删除已保存信息',
      'privacy.guardianDesc': '监护人应用只会收到确认危险短信或照护所需的摘要信息。您的密码及其他设置信息不会传递给监护人。',
      'privacy.guardianTitle': '监护人可见的信息',
      'privacy.photoDesc': '照片仅用于AI即时分析，分析结束后不会保留在服务器上。服务器只保存分析出的文字内容(摘要、待办事项)。',
      'privacy.photoTitle': '拍摄的文件照片',
      'privacy.pinDesc': '我们不会保存原始密码，只保存经过加密无法还原的数值，因此OnDam的工作人员也无法看到您的实际密码。',
      'privacy.pinTitle': '密码(PIN)',
      'privacy.syncDesc': '记录、日程、设置信息会保存在安全的服务器上，方便您在其他设备上继续使用。这些信息仅用于OnDam服务运营目的。',
      'privacy.syncTitle': '设备间同步',
      'privacy.title': '个人信息保管说明',
      'settings.accountTitle': '账户',
      'settings.deleteAccount': '退出账户',
      'settings.logout': '退出登录',
      'settings.privacyPolicy': '个人信息是如何保管的？',
  },
  vi: {
    ...
    'error.aiVoice': 'Hiện tại chưa thể phân tích. Hãy thử lại sau ít phút.',
      ...
      'deleteAccount.confirm': 'Xóa tài khoản',
      'deleteAccount.desc': 'Tất cả thông tin đã lưu như lịch sử, lịch trình, cài đặt sẽ bị xóa ngay lập tức và không thể khôi phục.',
      'deleteAccount.errorGeneric': 'Xử lý thất bại. Vui lòng thử lại sau.',
      'deleteAccount.errorPin': 'Mật khẩu không đúng.',
      'deleteAccount.pinLabel': 'Vui lòng nhập mật khẩu để xác nhận danh tính',
      'deleteAccount.title': 'Bạn có chắc chắn muốn xóa tài khoản không?',
      'onboard.guardian.desc': 'Dùng để báo ngay cho con cái khi nhận được tin nhắn nguy hiểm,<br>hoặc khi gọi điện bằng nút trợ giúp khẩn cấp.',
      'onboard.guardian.title': 'Bạn có muốn cho biết<br>thông tin con cái (người giám hộ) không?',
      'onboard.guardian.voice': 'Bạn có thể cho biết tên và số điện thoại của con cái hoặc người giám hộ để liên hệ khi có việc gấp không? Nếu không muốn, bạn có thể bỏ qua.',
      'onboard.login.errorInvalid': 'Số điện thoại hoặc mật khẩu không đúng',
      'onboard.login.errorLocked': 'Bạn đã nhập sai quá nhiều lần. Vui lòng thử lại sau 15 phút',
      'onboard.login.forgotPin': 'Bạn quên mật khẩu rồi à?',
      'onboard.resetPin.desc': 'Nhập tên và số điện thoại đã dùng khi đăng ký, chúng tôi sẽ gửi mã xác nhận qua tin nhắn.',
      'onboard.resetPin.errorOtpExpired': 'Mã xác nhận đã hết hạn. Vui lòng lấy lại mã mới',
      'onboard.resetPin.errorOtpInvalid': 'Mã xác nhận không đúng (còn {n} lần)',
      'onboard.resetPin.errorOtpLocked': 'Bạn đã nhập sai quá nhiều lần. Vui lòng thử lại từ đầu',
      'onboard.resetPin.errorSmsFailed': 'Gửi tin nhắn thất bại. Vui lòng thử lại sau',
      'onboard.resetPin.otpLabel': 'Mã xác nhận (6 số)',
      'onboard.resetPin.otpSentNotice': 'Đã gửi mã xác nhận',
      'onboard.resetPin.requestOtp': 'Nhận mã xác nhận',
      'onboard.resetPin.submit': 'Đặt lại mật khẩu',
      'onboard.resetPin.successNotice': 'Mật khẩu đã được đặt lại. Vui lòng đăng nhập bằng mật khẩu mới.',
      'onboard.resetPin.title': 'Đặt lại mật khẩu',
      'onboard.resetPin.voice': 'Nhập tên và số điện thoại, chúng tôi sẽ gửi mã xác nhận qua tin nhắn.',
      'onboard.signup.backHome': '← Trang chủ',
      'onboard.signup.desc': 'Vui lòng nhập số điện thoại và mật khẩu.<br>Nếu chưa có tài khoản, hệ thống sẽ tự động tạo mới cho bạn.',
      'onboard.signup.errorGeneric': 'Xử lý thất bại, vui lòng thử lại sau',
      'onboard.signup.errorPhone': 'Vui lòng kiểm tra lại số điện thoại',
      'onboard.signup.errorPinFormat': 'Vui lòng nhập mật khẩu gồm 4 chữ số',
      'onboard.signup.errorPinMismatch': 'Hai mật khẩu bạn nhập không khớp nhau',
      'onboard.signup.google': 'Đăng nhập bằng Google',
      'onboard.signup.kakao': 'Đăng nhập bằng Kakao',
      'onboard.signup.naver': 'Đăng nhập bằng Naver',
      'onboard.signup.or': 'Hoặc',
      'onboard.signup.phoneLabel': 'Số điện thoại',
      'onboard.signup.pinConfirmPlaceholder': 'Nhập lại mật khẩu',
      'onboard.signup.pinLabel': 'Mật khẩu',
      'onboard.signup.pinPlaceholder': 'Nhập mật khẩu',
      'onboard.signup.submit': 'Đăng nhập · Đăng ký',
      'onboard.signup.title': 'Đăng nhập hoặc<br>đăng ký tài khoản',
      'onboard.signup.voice': 'Vui lòng nhập số điện thoại và mật khẩu. Nếu chưa có tài khoản, hệ thống sẽ tự động tạo mới cho bạn.',
      'privacy.deleteDesc': 'Khi bạn nhấn "Xóa tài khoản" trong màn hình cài đặt, tài khoản cùng tất cả thông tin đã lưu sẽ bị xóa ngay khỏi máy chủ.',
      'privacy.deleteTitle': 'Xóa thông tin đã lưu',
      'privacy.guardianDesc': 'Ứng dụng người giám hộ chỉ nhận được thông tin tóm tắt cần thiết để xác nhận tin nhắn nguy hiểm hoặc chăm sóc. Mật khẩu và các thông tin cài đặt khác của bạn sẽ không được chia sẻ cho người giám hộ.',
      'privacy.guardianTitle': 'Thông tin người giám hộ có thể xem',
      'privacy.photoDesc': 'Ảnh chỉ được dùng để AI phân tích ngay lúc đó, sau khi phân tích xong sẽ không được lưu lại trên máy chủ. Máy chủ chỉ lưu nội dung chữ đã phân tích (tóm tắt, việc cần làm).',
      'privacy.photoTitle': 'Ảnh tài liệu đã chụp',
      'privacy.pinDesc': 'Chúng tôi không lưu mật khẩu gốc. Chỉ lưu giá trị đã mã hóa không thể nhận dạng được, nên nhân viên OnDam cũng không thể xem mật khẩu thật của bạn.',
      'privacy.pinTitle': 'Mật khẩu (PIN)',
      'privacy.syncDesc': 'Thông tin lịch sử, lịch trình, cài đặt được lưu trên máy chủ an toàn để bạn có thể tiếp tục sử dụng trên thiết bị khác. Thông tin này chỉ được dùng cho mục đích vận hành dịch vụ OnDam.',
      'privacy.syncTitle': 'Đồng bộ giữa các thiết bị',
      'privacy.title': 'Hướng dẫn lưu trữ thông tin cá nhân',
      'settings.accountTitle': 'Tài khoản',
      'settings.deleteAccount': 'Xóa tài khoản',
      'settings.logout': 'Đăng xuất',
      'settings.privacyPolicy': 'Thông tin cá nhân được lưu trữ như thế nào?',
      'coach.doc1.title': 'Chụp tài liệu để kiểm tra',
      'coach.doc1.desc': 'Khi chụp hóa đơn hay thông báo, chúng tôi sẽ cho bạn biết nội dung quan trọng và việc cần làm một cách dễ hiểu.',
      'coach.doc1.voice': 'Chụp tài liệu để kiểm tra là tính năng giúp bạn dễ dàng biết nội dung quan trọng trong hóa đơn hay thông báo.',
      'coach.doc2.title': 'Hãy thử tự chụp ảnh',
      'coach.doc2.desc': 'Hãy thử chụp tài liệu bằng camera.',
      'coach.doc2.voice': 'Hãy nhấn vào nút tự chụp ảnh.',
      'coach.doc3.title': 'Hãy nhấn nút chụp',
      'coach.doc3.desc': 'Canh cho tài liệu nằm giữa màn hình rồi nhấn nút.',
      'coach.doc3.voice': 'Hãy nhấn nút chụp.',
      'coach.emergency1.title': 'Khi khẩn cấp hãy nhấn nút này',
      'coach.emergency1.desc': 'Bạn có thể gọi ngay cho người giám hộ hoặc số 119·112·118. Hãy thử nhấn để xem trực tiếp, sau khi xem xong hãy chuyển sang bước tiếp theo.',
      'coach.emergency1.voice': 'Hãy thử nhấn nút trợ giúp. Nếu đã xem xong, hãy nhấn tiếp theo để chuyển bước.',
      'coach.finish.title': 'Giờ chỉ cần quay về trang chủ là xong',
      'coach.finish.desc': 'Nhấn nút ← Trang chủ để kết thúc hướng dẫn.',
      'coach.finish.voice': 'Nhấn nút trang chủ để kết thúc hướng dẫn.',
      'coach.fontsize.title': 'Hãy thử đổi cỡ chữ',
      'coach.fontsize.desc': 'Hãy chọn giữa Bình thường, Lớn, Rất lớn. Chọn xong hãy chuyển sang bước tiếp theo.',
      'coach.fontsize.voice': 'Hãy nhấn vào cỡ chữ. Chọn xong hãy nhấn tiếp theo để chuyển bước.',
      'coach.guardian.title': 'Hãy thử đăng ký thông tin người giám hộ',
      'coach.guardian.desc': 'Khi phát hiện tin nhắn nguy hiểm, bạn có thể báo ngay cho người giám hộ. Đây là mục tùy chọn nên nếu không muốn bạn có thể chuyển sang bước tiếp theo.',
      'coach.guardian.voice': 'Hãy thử nhập tên người giám hộ. Nếu không muốn, bạn có thể nhấn tiếp theo để bỏ qua.',
      'coach.helpback.title': 'Quay lại để kết thúc',
      'coach.helpback.desc': 'Hãy nhấn nút ← Quay lại.',
      'coach.helpback.voice': 'Hãy nhấn nút quay lại.',
      'coach.helplink.title': 'Cũng có hướng dẫn sử dụng',
      'coach.helplink.desc': 'Khi nào không rõ, bạn có thể xem lại bất cứ lúc nào.',
      'coach.helplink.voice': 'Hãy nhấn vào hướng dẫn sử dụng.',
      'coach.history1.title': 'Bạn cũng có thể xem lịch sử',
      'coach.history1.desc': 'Bạn có thể xem lại tất cả tài liệu và tin nhắn đã kiểm tra từ trước đến nay.',
      'coach.history1.voice': 'Hãy thử nhấn nút lịch sử bên dưới.',
      'coach.history2.title': 'Bây giờ hãy quay lại trang chủ',
      'coach.history2.desc': 'Bạn có thể quay lại bất cứ lúc nào bằng cách nhấn nút ← Trang chủ.',
      'coach.history2.voice': 'Hãy thử nhấn nút trang chủ để quay lại.',
      'coach.info1.title': 'Cũng có thông tin hữu ích nên biết',
      'coach.info1.desc': 'Chúng tôi sẽ hướng dẫn các thông tin hữu ích như lương hưu cơ bản, khám sức khỏe.',
      'coach.info1.voice': 'Hãy thử nhấn vào thông tin hữu ích nên biết.',
      'coach.info2.title': 'Xem xong rồi thì quay lại trang chủ nhé',
      'coach.info2.desc': 'Hãy nhấn nút quay lại trang chủ.',
      'coach.info2.voice': 'Hãy nhấn nút quay lại trang chủ.',
      'coach.welfare1.title': 'Tìm nhà sinh hoạt cộng đồng gần đây',
      'coach.welfare1.desc': 'Chúng tôi sẽ tìm nhà sinh hoạt cộng đồng gần bạn dựa trên vị trí hiện tại hoặc khu vực đã nhập.',
      'coach.welfare1.voice': 'Tìm nhà sinh hoạt cộng đồng gần đây là tính năng cho biết vị trí các nhà sinh hoạt cộng đồng quanh bạn.',
      'coach.welfare2.title': 'Bây giờ hãy quay lại màn hình chính',
      'coach.welfare2.desc': 'Hãy nhấn nút quay lại màn hình chính.',
      'coach.welfare2.voice': 'Hãy nhấn nút quay lại màn hình chính.',
      'coach.voice1.title': 'Bạn cũng có thể nghe hướng dẫn bằng giọng nói',
      'coach.voice1.desc': 'Nhấn nút này để nghe lại hướng dẫn màn hình.',
      'coach.voice1.voice': 'Hãy thử nhấn nút nghe hướng dẫn bằng giọng nói.',
      'coach.settingsIntro.title': 'Cùng xem thêm mục khác',
      'coach.settingsIntro.desc': 'Bạn có thể xem thông tin cá nhân, lịch sử, cài đặt ở đây.',
      'coach.settingsIntro.voice': 'Hãy thử nhấn nút xem thêm bên dưới.',
      'coach.moreToSettings.title': 'Hãy thử nhấn vào cài đặt',
      'coach.moreToSettings.desc': 'Bạn có thể thay đổi cỡ chữ, tốc độ giọng nói, thông tin người giám hộ.',
      'coach.moreToSettings.voice': 'Hãy thử nhấn vào cài đặt.',
      'coach.rate.title': 'Bạn cũng có thể thay đổi tốc độ giọng nói',
      'coach.rate.desc': 'Hãy chọn tốc độ đọc phù hợp với bạn. Chọn xong hãy chuyển sang bước tiếp theo.',
      'coach.rate.voice': 'Hãy nhấn vào tốc độ đọc giọng nói. Chọn xong hãy nhấn tiếp theo để chuyển bước.',
      'coach.language.title': 'Bạn cũng có thể đổi ngôn ngữ',
      'coach.language.desc': 'Hãy chọn trong số tiếng Trung, tiếng Việt, tiếng Thái, tiếng Uzbek. Chọn xong hãy chuyển sang bước tiếp theo.',
      'coach.language.voice': 'Hãy thử nhấn vào cài đặt ngôn ngữ. Chọn xong hãy nhấn tiếp theo để chuyển bước.',
      'coach.moreHelp.title': 'Ở đây bạn cũng có thể xem các tính năng khác',
      'coach.moreHelp.desc': 'Hãy thử nhấn vào thông tin·lịch sử·cài đặt bên dưới.',
      'coach.moreHelp.voice': 'Bạn cũng có thể xem các tính năng khác ở menu bên dưới.',
      'coach.next': 'Chuyển sang bước tiếp theo',
      'coach.skipTutorial': 'Bỏ qua hướng dẫn',
      'coach.sms1.title': 'Kiểm tra tin nhắn đáng ngờ',
      'coach.sms1.desc': 'Kiểm tra xem tin nhắn nhận được có nguy hiểm hay an toàn, và cho biết cả lý do nguy hiểm.',
      'coach.sms1.voice': 'Kiểm tra tin nhắn đáng ngờ là tính năng cho biết tin nhắn nhận được có nguy hiểm hay an toàn.',
      'coach.smsPermission.title': 'Vui lòng cho phép đọc tin nhắn',
      'coach.smsPermission.desc': 'Nếu cho phép, chúng tôi sẽ hiển thị ngay tin nhắn gần đây.',
      'coach.smsPermission.voice': 'Hãy nhấn cho phép.',
      'coach.sms2.title': 'Hãy nhấn vào tin nhắn này để kiểm tra',
      'coach.sms2.desc': 'Chỉ cần chạm một lần là có thể kiểm tra ngay.',
      'coach.sms2.voice': 'Hãy thử nhấn vào tin nhắn để kiểm tra.',
  },
  th: {
    ...
    'error.aiVoice': 'ตอนนี้ยังวิเคราะห์ไม่ได้ กรุณาลองใหม่อีกครั้งในภายหลัง',
      ...
      'deleteAccount.confirm': 'ยืนยันการลบบัญชี',
      'deleteAccount.desc': 'ข้อมูลทั้งหมดที่บันทึกไว้ เช่น ประวัติ ตารางนัด การตั้งค่า จะถูกลบทันทีและไม่สามารถกู้คืนได้',
      'deleteAccount.errorGeneric': 'ดำเนินการไม่สำเร็จ กรุณาลองใหม่อีกครั้งภายหลัง',
      'deleteAccount.errorPin': 'รหัสผ่านไม่ถูกต้อง',
      'deleteAccount.pinLabel': 'กรุณากรอกรหัสผ่านเพื่อยืนยันตัวตน',
      'deleteAccount.title': 'ต้องการลบบัญชีจริงหรือไม่?',
      'onboard.guardian.desc': 'ใช้แจ้งบุตรหลานทันทีเมื่อได้รับข้อความอันตราย<br>หรือใช้เมื่อกดปุ่มขอความช่วยเหลือฉุกเฉินเพื่อโทรออก',
      'onboard.guardian.title': 'ขอทราบข้อมูล<br>บุตรหลาน(ผู้ดูแล)ด้วยได้ไหม?',
      'onboard.guardian.voice': 'ขอทราบชื่อและเบอร์โทรศัพท์ของบุตรหลานหรือผู้ดูแลที่จะแจ้งเมื่อมีเหตุด่วนได้ไหม? หากไม่ต้องการสามารถข้ามได้',
      'onboard.login.errorInvalid': 'เบอร์โทรศัพท์หรือรหัสผ่านไม่ถูกต้อง',
      'onboard.login.errorLocked': 'กรอกผิดหลายครั้งเกินไป กรุณาลองใหม่อีกครั้งใน 15 นาที',
      'onboard.login.forgotPin': 'ลืมรหัสผ่านใช่ไหม?',
      'onboard.resetPin.desc': 'กรอกชื่อและเบอร์โทรศัพท์ที่ใช้ตอนสมัครสมาชิก แล้วเราจะส่งรหัสยืนยันทางข้อความ',
      'onboard.resetPin.errorOtpExpired': 'รหัสยืนยันหมดอายุแล้ว กรุณาขอรหัสใหม่',
      'onboard.resetPin.errorOtpInvalid': 'รหัสยืนยันไม่ถูกต้อง (เหลืออีก {n} ครั้ง)',
      'onboard.resetPin.errorOtpLocked': 'กรอกผิดหลายครั้งเกินไป กรุณาเริ่มใหม่ตั้งแต่ต้น',
      'onboard.resetPin.errorSmsFailed': 'ส่งข้อความไม่สำเร็จ กรุณาลองใหม่อีกครั้งภายหลัง',
      'onboard.resetPin.otpLabel': 'รหัสยืนยัน (6 หลัก)',
      'onboard.resetPin.otpSentNotice': 'ส่งรหัสยืนยันแล้ว',
      'onboard.resetPin.requestOtp': 'รับรหัสยืนยัน',
      'onboard.resetPin.submit': 'ตั้งรหัสผ่านใหม่',
      'onboard.resetPin.successNotice': 'ตั้งรหัสผ่านใหม่เรียบร้อยแล้ว กรุณาเข้าสู่ระบบด้วยรหัสผ่านใหม่',
      'onboard.resetPin.title': 'ตั้งรหัสผ่านใหม่',
      'onboard.resetPin.voice': 'กรอกชื่อและเบอร์โทรศัพท์ แล้วเราจะส่งรหัสยืนยันทางข้อความ',
      'onboard.signup.backHome': '← กลับหน้าหลัก',
      'onboard.signup.desc': 'กรุณากรอกเบอร์โทรศัพท์และรหัสผ่าน<br>หากยังไม่มีบัญชี ระบบจะสร้างบัญชีใหม่ให้อัตโนมัติ',
      'onboard.signup.errorGeneric': 'ดำเนินการไม่สำเร็จ กรุณาลองใหม่อีกครั้งภายหลัง',
      'onboard.signup.errorPhone': 'กรุณาตรวจสอบเบอร์โทรศัพท์อีกครั้ง',
      'onboard.signup.errorPinFormat': 'กรุณากรอกรหัสผ่านเป็นตัวเลข 4 หลัก',
      'onboard.signup.errorPinMismatch': 'รหัสผ่านที่กรอกไม่ตรงกัน',
      'onboard.signup.google': 'เข้าสู่ระบบด้วย Google',
      'onboard.signup.kakao': 'เข้าสู่ระบบด้วย Kakao',
      'onboard.signup.naver': 'เข้าสู่ระบบด้วย Naver',
      'onboard.signup.or': 'หรือ',
      'onboard.signup.phoneLabel': 'เบอร์โทรศัพท์',
      'onboard.signup.pinConfirmPlaceholder': 'กรอกรหัสผ่านอีกครั้ง',
      'onboard.signup.pinLabel': 'รหัสผ่าน',
      'onboard.signup.pinPlaceholder': 'กรอกรหัสผ่าน',
      'onboard.signup.submit': 'เข้าสู่ระบบ · สมัครสมาชิก',
      'onboard.signup.title': 'เข้าสู่ระบบหรือ<br>สมัครสมาชิก',
      'onboard.signup.voice': 'กรุณากรอกเบอร์โทรศัพท์และรหัสผ่าน หากยังไม่มีบัญชี ระบบจะสร้างบัญชีใหม่ให้อัตโนมัติ',
      'privacy.deleteDesc': 'เมื่อกดปุ่ม "ลบบัญชี" ในหน้าตั้งค่า ข้อมูลทั้งหมดที่บันทึกไว้พร้อมกับบัญชีจะถูกลบออกจากเซิร์ฟเวอร์ทันที',
      'privacy.deleteTitle': 'ลบข้อมูลที่บันทึกไว้',
      'privacy.guardianDesc': 'แอปผู้ดูแลจะได้รับเพียงข้อมูลสรุปที่จำเป็นสำหรับตรวจสอบข้อความอันตรายหรือการดูแลเท่านั้น รหัสผ่านหรือข้อมูลการตั้งค่าอื่น ๆ ของคุณจะไม่ถูกส่งให้ผู้ดูแล',
      'privacy.guardianTitle': 'ข้อมูลที่ผู้ดูแลมองเห็นได้',
      'privacy.photoDesc': 'รูปภาพจะถูกใช้เพื่อให้ AI วิเคราะห์ในขณะนั้นเท่านั้น เมื่อวิเคราะห์เสร็จแล้วจะไม่เก็บไว้ในเซิร์ฟเวอร์ เซิร์ฟเวอร์จะเก็บเฉพาะเนื้อหาตัวอักษรที่วิเคราะห์แล้ว (สรุป·สิ่งที่ต้องทำ)',
      'privacy.photoTitle': 'รูปถ่ายเอกสารที่ถ่ายไว้',
      'privacy.pinDesc': 'เราไม่เก็บรหัสผ่านต้นฉบับ จะเก็บเฉพาะค่าที่เข้ารหัสจนไม่สามารถอ่านได้ ดังนั้นแม้แต่พนักงานของ OnDam ก็ไม่สามารถเห็นรหัสผ่านจริงของคุณได้',
      'privacy.pinTitle': 'รหัสผ่าน (PIN)',
      'privacy.syncDesc': 'ข้อมูลประวัติ ตารางนัด การตั้งค่า จะถูกบันทึกไว้ในเซิร์ฟเวอร์ที่ปลอดภัย เพื่อให้สามารถใช้งานต่อเนื่องได้จากอุปกรณ์เครื่องอื่น ข้อมูลนี้ใช้เพื่อวัตถุประสงค์ในการให้บริการ OnDam เท่านั้น',
      'privacy.syncTitle': 'การซิงค์ข้อมูลระหว่างอุปกรณ์',
      'privacy.title': 'คำแนะนำการเก็บรักษาข้อมูลส่วนบุคคล',
      'settings.accountTitle': 'บัญชี',
      'settings.deleteAccount': 'ลบบัญชี',
      'settings.logout': 'ออกจากระบบ',
      'settings.privacyPolicy': 'ข้อมูลส่วนบุคคลถูกเก็บรักษาอย่างไร?',
      'coach.doc1.title': 'ถ่ายเอกสารเพื่อตรวจสอบ',
      'coach.doc1.desc': 'เมื่อถ่ายใบแจ้งหนี้หรือเอกสารแจ้งเตือน เราจะบอกเนื้อหาสำคัญและสิ่งที่ต้องทำให้เข้าใจง่าย',
      'coach.doc1.voice': 'ถ่ายเอกสารเพื่อตรวจสอบ คือฟังก์ชันที่ช่วยบอกเนื้อหาสำคัญของใบแจ้งหนี้หรือเอกสารแจ้งเตือนให้เข้าใจง่าย',
      'coach.doc2.title': 'ลองถ่ายภาพด้วยตัวเองดูนะ',
      'coach.doc2.desc': 'ลองถ่ายเอกสารด้วยกล้องดูนะ',
      'coach.doc2.voice': 'ลองกดปุ่มถ่ายภาพดูนะ',
      'coach.doc3.title': 'กรุณากดปุ่มถ่ายภาพ',
      'coach.doc3.desc': 'จัดให้เอกสารอยู่กึ่งกลางจอแล้วกดปุ่ม',
      'coach.doc3.voice': 'กรุณากดปุ่มถ่ายภาพ',
      'coach.emergency1.title': 'ตอนฉุกเฉินให้กดปุ่มนี้',
      'coach.emergency1.desc': 'สามารถโทรหาผู้ดูแลหรือ 119·112·118 ได้ทันที ลองกดดูเพื่อตรวจสอบ เมื่อดูเสร็จแล้วให้ไปขั้นตอนถัดไป',
      'coach.emergency1.voice': 'ลองกดปุ่มขอความช่วยเหลือดูนะ เมื่อตรวจสอบแล้วให้กดถัดไปเพื่อไปขั้นตอนต่อไป',
      'coach.finish.title': 'ตอนนี้แค่กลับไปหน้าหลักก็เสร็จแล้ว',
      'coach.finish.desc': 'กดปุ่ม ← กลับหน้าหลัก เพื่อจบคำแนะนำ',
      'coach.finish.voice': 'กดปุ่มกลับหน้าหลักเพื่อจบคำแนะนำ',
      'coach.fontsize.title': 'ลองเปลี่ยนขนาดตัวอักษรดูนะ',
      'coach.fontsize.desc': 'เลือกระหว่าง ปกติ ใหญ่ ใหญ่มาก เมื่อเลือกเสร็จแล้วให้ไปขั้นตอนถัดไป',
      'coach.fontsize.voice': 'ลองกดขนาดตัวอักษรดูนะ เมื่อเลือกเสร็จแล้วให้กดถัดไปเพื่อไปขั้นตอนต่อไป',
      'coach.guardian.title': 'ลองลงทะเบียนข้อมูลผู้ดูแลดูนะ',
      'coach.guardian.desc': 'เมื่อพบข้อความอันตราย สามารถแจ้งผู้ดูแลได้ทันที เป็นรายการเสริมดังนั้นหากไม่ต้องการก็ไปขั้นตอนถัดไปได้',
      'coach.guardian.voice': 'ลองกรอกชื่อผู้ดูแลดูนะ หากไม่ต้องการสามารถกดถัดไปเพื่อข้ามได้',
      'coach.helpback.title': 'กลับไปเพื่อจบขั้นตอน',
      'coach.helpback.desc': 'กรุณากดปุ่ม ← กลับ',
      'coach.helpback.voice': 'กรุณากดปุ่มกลับ',
      'coach.helplink.title': 'มีคำแนะนำการใช้งานด้วยนะ',
      'coach.helplink.desc': 'เมื่อสับสนสามารถกลับมาดูได้ตลอดเวลา',
      'coach.helplink.voice': 'ลองกดคำแนะนำการใช้งานดูนะ',
      'coach.history1.title': 'สามารถดูประวัติได้ด้วยนะ',
      'coach.history1.desc': 'สามารถรวบรวมดูเอกสารและข้อความที่ตรวจสอบมาจนถึงตอนนี้ได้',
      'coach.history1.voice': 'ลองกดปุ่มประวัติด้านล่างดูนะ',
      'coach.history2.title': 'ลองกลับไปหน้าหลักอีกครั้งดูนะ',
      'coach.history2.desc': 'กดปุ่ม ← กลับหน้าหลัก เพื่อกลับไปได้ทุกเมื่อ',
      'coach.history2.voice': 'ลองกดปุ่มกลับหน้าหลักเพื่อกลับไปดูนะ',
      'coach.info1.title': 'มีข้อมูลดี ๆ ที่ควรรู้ด้วยนะ',
      'coach.info1.desc': 'เราจะแนะนำข้อมูลที่เป็นประโยชน์ เช่น เงินบำนาญพื้นฐาน การตรวจสุขภาพ',
      'coach.info1.voice': 'ลองกดข้อมูลดี ๆ ที่ควรรู้ดูนะ',
      'coach.info2.title': 'ดูเสร็จแล้วก็กลับไปหน้าหลักนะ',
      'coach.info2.desc': 'กรุณากดปุ่มกลับไปหน้าหลัก',
      'coach.info2.voice': 'กรุณากดปุ่มกลับไปหน้าหลัก',
      'coach.welfare1.title': 'ค้นหาศูนย์ผู้สูงอายุใกล้บ้าน',
      'coach.welfare1.desc': 'จะค้นหาศูนย์ผู้สูงอายุที่ใกล้ตามตำแหน่งปัจจุบันหรือพื้นที่ที่กรอกไว้ให้',
      'coach.welfare1.voice': 'ค้นหาศูนย์ผู้สูงอายุใกล้บ้าน คือฟังก์ชันที่บอกตำแหน่งศูนย์ผู้สูงอายุรอบตัวคุณ',
      'coach.welfare2.title': 'ลองกลับไปหน้าหลักดูนะ',
      'coach.welfare2.desc': 'กรุณากดปุ่มกลับไปหน้าหลัก',
      'coach.welfare2.voice': 'กรุณากดปุ่มกลับไปหน้าหลัก',
      'coach.voice1.title': 'สามารถรับฟังคำแนะนำด้วยเสียงได้ด้วยนะ',
      'coach.voice1.desc': 'กดปุ่มนี้เพื่อฟังคำแนะนำหน้าจออีกครั้ง',
      'coach.voice1.voice': 'ลองกดปุ่มรับฟังด้วยเสียงดูนะ',
      'coach.settingsIntro.title': 'มาดูเมนูอื่น ๆ กันด้วยนะ',
      'coach.settingsIntro.desc': 'สามารถดูข้อมูลส่วนตัว ประวัติ การตั้งค่าได้ที่นี่',
      'coach.settingsIntro.voice': 'ลองกดปุ่มเมนูเพิ่มเติมด้านล่างดูนะ',
      'coach.moreToSettings.title': 'ลองกดการตั้งค่าดูนะ',
      'coach.moreToSettings.desc': 'สามารถเปลี่ยนขนาดตัวอักษร ความเร็วเสียง ข้อมูลผู้ดูแลได้',
      'coach.moreToSettings.voice': 'ลองกดการตั้งค่าดูนะ',
      'coach.rate.title': 'สามารถเปลี่ยนความเร็วเสียงได้ด้วยนะ',
      'coach.rate.desc': 'เลือกความเร็วในการอ่านให้เหมาะกับคุณ เมื่อเลือกเสร็จแล้วให้ไปขั้นตอนถัดไป',
      'coach.rate.voice': 'ลองกดความเร็วในการอ่านออกเสียงดูนะ เมื่อเลือกเสร็จแล้วให้กดถัดไปเพื่อไปขั้นตอนต่อไป',
      'coach.language.title': 'สามารถเปลี่ยนภาษาได้ด้วยนะ',
      'coach.language.desc': 'เลือกระหว่างภาษาจีน เวียดนาม ไทย อุซเบก เมื่อเลือกเสร็จแล้วให้ไปขั้นตอนถัดไป',
      'coach.language.voice': 'ลองกดการตั้งค่าภาษาดูนะ เมื่อเลือกเสร็จแล้วให้กดถัดไปเพื่อไปขั้นตอนต่อไป',
      'coach.moreHelp.title': 'ที่นี่สามารถดูฟังก์ชันอื่น ๆ ได้ด้วยนะ',
      'coach.moreHelp.desc': 'ลองกดข้อมูล·ประวัติ·การตั้งค่าด้านล่างดูนะ',
      'coach.moreHelp.voice': 'สามารถดูฟังก์ชันอื่น ๆ ได้ที่เมนูด้านล่าง',
      'coach.next': 'ไปขั้นตอนถัดไป',
      'coach.skipTutorial': 'ข้ามคำแนะนำการใช้งาน',
      'coach.sms1.title': 'ตรวจสอบข้อความน่าสงสัย',
      'coach.sms1.desc': 'ตรวจสอบว่าข้อความที่ได้รับอันตรายหรือปลอดภัย พร้อมบอกเหตุผลที่อันตรายด้วย',
      'coach.sms1.voice': 'ตรวจสอบข้อความน่าสงสัย คือฟังก์ชันที่บอกว่าข้อความที่ได้รับอันตรายหรือปลอดภัย',
      'coach.smsPermission.title': 'กรุณาอนุญาตให้อ่านข้อความ',
      'coach.smsPermission.desc': 'หากอนุญาต จะแสดงข้อความล่าสุดให้ทันที',
      'coach.smsPermission.voice': 'กรุณากดอนุญาต',
      'coach.sms2.title': 'กดข้อความนี้เพื่อตรวจสอบดูนะ',
      'coach.sms2.desc': 'แตะเพียงครั้งเดียวก็สามารถตรวจสอบได้ทันที',
      'coach.sms2.voice': 'ลองกดข้อความเพื่อตรวจสอบดูนะ',
  },
  uz: {
    ...
    'error.aiVoice': "Hozir tahlil qilib bo'lmadi. Birozdan so'ng qayta urinib ko'ring.",
      ...
      'deleteAccount.confirm': 'Hisobni o\'chirish',
      'deleteAccount.desc': 'Tarix, jadval, sozlamalar kabi saqlangan barcha ma\'lumotlar darhol o\'chiriladi va qaytarib bo\'lmaydi.',
      'deleteAccount.errorGeneric': 'Amalga oshmadi. Birozdan keyin qayta urinib ko\'ring.',
      'deleteAccount.errorPin': 'Parol noto\'g\'ri.',
      'deleteAccount.pinLabel': 'Shaxsingizni tasdiqlash uchun parolni kiriting',
      'deleteAccount.title': 'Hisobingizni haqiqatan o\'chirmoqchimisiz?',
      'onboard.guardian.desc': 'Xavfli SMS kelganda farzandingizga darhol xabar berish uchun,<br>yoki favqulodda yordam tugmasi orqali qo\'ng\'iroq qilishda ishlatiladi.',
      'onboard.guardian.title': 'Farzand (vasiy) ma\'lumotlarini ham<br>bera olasizmi?',
      'onboard.guardian.voice': 'Shoshilinch vaziyatda xabar beriladigan farzand yoki vasiyning ismi va telefon raqamini bera olasizmi? Xohlamasangiz, o\'tkazib yuborishingiz mumkin.',
      'onboard.login.errorInvalid': 'Telefon raqami yoki parol noto\'g\'ri',
      'onboard.login.errorLocked': 'Juda ko\'p marta xato kiritdingiz. 15 daqiqadan so\'ng qayta urinib ko\'ring',
      'onboard.login.forgotPin': 'Parolingizni unutdingizmi?',
      'onboard.resetPin.desc': 'Ro\'yxatdan o\'tishda ishlatgan ism va telefon raqamingizni kiriting, tasdiqlash kodini SMS orqali yuboramiz.',
      'onboard.resetPin.errorOtpExpired': 'Tasdiqlash kodi muddati tugadi. Qayta oling',
      'onboard.resetPin.errorOtpInvalid': 'Tasdiqlash kodi noto\'g\'ri ({n} marta qoldi)',
      'onboard.resetPin.errorOtpLocked': 'Juda ko\'p marta xato kiritdingiz. Boshidan qayta urinib ko\'ring',
      'onboard.resetPin.errorSmsFailed': 'SMS yuborilmadi. Birozdan keyin qayta urinib ko\'ring',
      'onboard.resetPin.otpLabel': 'Tasdiqlash kodi (6 xonali)',
      'onboard.resetPin.otpSentNotice': 'Tasdiqlash kodi yuborildi',
      'onboard.resetPin.requestOtp': 'Tasdiqlash kodini olish',
      'onboard.resetPin.submit': 'Qayta o\'rnatish',
      'onboard.resetPin.successNotice': 'Parol qayta o\'rnatildi. Yangi parol bilan tizimga kiring.',
      'onboard.resetPin.title': 'Parolni qayta o\'rnatish',
      'onboard.resetPin.voice': 'Ism va telefon raqamingizni kiriting, tasdiqlash kodini SMS orqali yuboramiz.',
      'onboard.signup.backHome': '← Bosh sahifa',
      'onboard.signup.desc': 'Telefon raqami va parolni kiriting.<br>Agar hisobingiz bo\'lmasa, avtomatik ravishda yangi hisob yaratiladi.',
      'onboard.signup.errorGeneric': 'Amalga oshmadi, birozdan keyin qayta urinib ko\'ring',
      'onboard.signup.errorPhone': 'Telefon raqamini qayta tekshiring',
      'onboard.signup.errorPinFormat': 'Parolni 4 xonali raqam bilan kiriting',
      'onboard.signup.errorPinMismatch': 'Kiritgan parollaringiz bir-biriga mos kelmadi',
      'onboard.signup.google': 'Google orqali kirish',
      'onboard.signup.kakao': 'Kakao orqali kirish',
      'onboard.signup.naver': 'Naver orqali kirish',
      'onboard.signup.or': 'Yoki',
      'onboard.signup.phoneLabel': 'Telefon raqami',
      'onboard.signup.pinConfirmPlaceholder': 'Parolni qayta kiriting',
      'onboard.signup.pinLabel': 'Parol',
      'onboard.signup.pinPlaceholder': 'Parolni kiriting',
      'onboard.signup.submit': 'Kirish · Ro\'yxatdan o\'tish',
      'onboard.signup.title': 'Kirish yoki<br>ro\'yxatdan o\'tish',
      'onboard.signup.voice': 'Telefon raqami va parolni kiriting. Agar hisobingiz bo\'lmasa, avtomatik ravishda yangi hisob yaratiladi.',
      'privacy.deleteDesc': 'Sozlamalar sahifasidagi "Hisobni o\'chirish" tugmasini bosganda, hisobingiz va u bilan saqlangan barcha ma\'lumotlar serverdan darhol o\'chiriladi.',
      'privacy.deleteTitle': 'Saqlangan ma\'lumotlarni o\'chirish',
      'privacy.guardianDesc': 'Vasiy ilovasiga faqat xavfli SMS-larni tekshirish yoki g\'amxo\'rlik uchun zarur bo\'lgan qisqacha ma\'lumot yuboriladi. Parolingiz yoki boshqa sozlamalar ma\'lumotlari vasiyga yuborilmaydi.',
      'privacy.guardianTitle': 'Vasiyga ko\'rinadigan ma\'lumotlar',
      'privacy.photoDesc': 'Rasm faqat o\'sha zahoti AI tomonidan tahlil qilish uchun ishlatiladi, tahlildan so\'ng serverda saqlanmaydi. Serverda faqat tahlil qilingan matn mazmuni (xulosa, bajarilishi kerak bo\'lgan ishlar) saqlanadi.',
      'privacy.photoTitle': 'Suratga olingan hujjat rasmi',
      'privacy.pinDesc': 'Asl parolni saqlamaymiz. Faqat tanib bo\'lmaydigan darajada shifrlangan qiymat saqlanadi, shuning uchun OnDam xodimlari ham haqiqiy parolingizni ko\'ra olmaydi.',
      'privacy.pinTitle': 'Parol (PIN)',
      'privacy.syncDesc': 'Tarix, jadval, sozlamalar ma\'lumotlari boshqa qurilmada ham davom ettirish uchun xavfsiz serverda saqlanadi. Bu ma\'lumot faqat OnDam xizmatini yuritish maqsadida ishlatiladi.',
      'privacy.syncTitle': 'Qurilmalar o\'rtasida sinxronlash',
      'privacy.title': 'Shaxsiy ma\'lumotlarni saqlash haqida',
      'settings.accountTitle': 'Hisob',
      'settings.deleteAccount': 'Hisobni o\'chirish',
      'settings.logout': 'Chiqish',
      'settings.privacyPolicy': 'Shaxsiy ma\'lumotlar qanday saqlanadi?',
      'coach.doc1.title': 'Hujjatni suratga olib tekshirish',
      'coach.doc1.desc': 'Hisob-faktura yoki xabarnomani suratga olsangiz, muhim mazmuni va bajarilishi kerak bo\'lgan ishlarni oson tushuntirib beramiz.',
      'coach.doc1.voice': 'Hujjatni suratga olib tekshirish — hisob-faktura yoki xabarnomaning muhim mazmunini oson tushuntirib beruvchi funksiya.',
      'coach.doc2.title': 'Keling, o\'zingiz suratga olib ko\'ring',
      'coach.doc2.desc': 'Kamera bilan hujjatni suratga oling.',
      'coach.doc2.voice': 'O\'zingiz suratga olish tugmasini bosing.',
      'coach.doc3.title': 'Suratga olish tugmasini bosing',
      'coach.doc3.desc': 'Hujjatni ekran markazida joylashtirib, tugmani bosing.',
      'coach.doc3.voice': 'Suratga olish tugmasini bosing.',
      'coach.emergency1.title': 'Shoshilinch holatda shu tugmani bosing',
      'coach.emergency1.desc': 'Vasiyga yoki 119·112·118 raqamlariga darhol qo\'ng\'iroq qila olasiz. Bosib ko\'rib tekshiring, ko\'rib bo\'lgach keyingisiga o\'ting.',
      'coach.emergency1.voice': 'Yordam tugmasini bosib ko\'ring. Tekshirib bo\'lgach, keyingisiga o\'tish uchun bosing.',
      'coach.finish.title': 'Endi faqat bosh sahifaga qaytsangiz tamom',
      'coach.finish.desc': 'Yo\'riqnomani tugatish uchun ← Bosh sahifa tugmasini bosing.',
      'coach.finish.voice': 'Yo\'riqnomani tugatish uchun bosh sahifa tugmasini bosing.',
      'coach.fontsize.title': 'Shrift hajmini o\'zgartirib ko\'ring',
      'coach.fontsize.desc': 'Oddiy, Katta, Juda katta orasidan tanlang. Tanlab bo\'lgach keyingisiga o\'ting.',
      'coach.fontsize.voice': 'Shrift hajmini bosib ko\'ring. Tanlab bo\'lgach keyingisiga o\'tish uchun bosing.',
      'coach.guardian.title': 'Vasiy ma\'lumotlarini ro\'yxatdan o\'tkazib ko\'ring',
      'coach.guardian.desc': 'Xavfli SMS aniqlansa, vasiyga darhol xabar bera olasiz. Bu ixtiyoriy band, xohlamasangiz keyingisiga o\'tishingiz mumkin.',
      'coach.guardian.voice': 'Vasiy ismini kiritib ko\'ring. Xohlamasangiz, o\'tkazib yuborish uchun keyingisini bosing.',
      'coach.helpback.title': 'Orqaga qaytib yakunlaymiz',
      'coach.helpback.desc': '← Orqaga tugmasini bosing.',
      'coach.helpback.voice': 'Orqaga tugmasini bosing.',
      'coach.helplink.title': 'Foydalanish bo\'yicha qo\'llanma ham bor',
      'coach.helplink.desc': 'Chalkashib ketsangiz, istalgan vaqtda qayta ko\'rishingiz mumkin.',
      'coach.helplink.voice': 'Foydalanish bo\'yicha qo\'llanmani bosib ko\'ring.',
      'coach.history1.title': 'Tarixni ham ko\'rishingiz mumkin',
      'coach.history1.desc': 'Hozirgacha tekshirilgan hujjat va SMS-larni to\'plab ko\'rishingiz mumkin.',
      'coach.history1.voice': 'Pastdagi tarix tugmasini bosib ko\'ring.',
      'coach.history2.title': 'Yana bosh sahifaga qaytamiz',
      'coach.history2.desc': '← Bosh sahifa tugmasi orqali istalgan vaqtda qaytishingiz mumkin.',
      'coach.history2.voice': 'Qaytish uchun bosh sahifa tugmasini bosib ko\'ring.',
      'coach.info1.title': 'Bilib qo\'yish kerak bo\'lgan foydali ma\'lumotlar ham bor',
      'coach.info1.desc': 'Asosiy pensiya, sog\'liqni tekshirish kabi foydali ma\'lumotlarni ko\'rsatib beramiz.',
      'coach.info1.voice': 'Bilib qo\'yish kerak bo\'lgan foydali ma\'lumotlarni bosib ko\'ring.',
      'coach.info2.title': 'Ko\'rib bo\'lgach bosh sahifaga qaytamiz',
      'coach.info2.desc': 'Bosh sahifaga qaytish tugmasini bosing.',
      'coach.info2.voice': 'Bosh sahifaga qaytish tugmasini bosing.',
      'coach.welfare1.title': 'Yaqin atrofdagi keksalar uyini topish',
      'coach.welfare1.desc': 'Hozirgi joylashuv yoki kiritilgan hudud asosida yaqin keksalar uyini topib beramiz.',
      'coach.welfare1.voice': 'Yaqin atrofdagi keksalar uyini topish — atrofingizdagi keksalar uylari joylashuvini ko\'rsatuvchi funksiya.',
      'coach.welfare2.title': 'Yana bosh sahifaga qaytamiz',
      'coach.welfare2.desc': 'Bosh sahifaga qaytish tugmasini bosing.',
      'coach.welfare2.voice': 'Bosh sahifaga qaytish tugmasini bosing.',
      'coach.voice1.title': 'Ovozli yo\'riqnomani ham eshitishingiz mumkin',
      'coach.voice1.desc': 'Ushbu tugmani bossangiz, ekran yo\'riqnomasini qayta eshitasiz.',
      'coach.voice1.voice': 'Ovozli yo\'riqnoma tugmasini bosib ko\'ring.',
      'coach.settingsIntro.title': 'Boshqa bo\'limlarni ham ko\'ramiz',
      'coach.settingsIntro.desc': 'Bu yerda shaxsiy ma\'lumot, tarix, sozlamalarni ko\'rishingiz mumkin.',
      'coach.settingsIntro.voice': 'Pastdagi «Ko\'proq» tugmasini bosib ko\'ring.',
      'coach.moreToSettings.title': 'Sozlamalarni bosib ko\'ring',
      'coach.moreToSettings.desc': 'Shrift hajmi, ovoz tezligi, vasiy ma\'lumotlarini o\'zgartirishingiz mumkin.',
      'coach.moreToSettings.voice': 'Sozlamalarni bosib ko\'ring.',
      'coach.rate.title': 'Ovoz tezligini ham o\'zgartirishingiz mumkin',
      'coach.rate.desc': 'O\'zingizga qulay o\'qish tezligini tanlang. Tanlab bo\'lgach keyingisiga o\'ting.',
      'coach.rate.voice': 'Ovoz o\'qish tezligini bosib ko\'ring. Tanlab bo\'lgach keyingisiga o\'tish uchun bosing.',
      'coach.language.title': 'Tilni ham o\'zgartirishingiz mumkin',
      'coach.language.desc': 'Xitoy, Vetnam, Tay, O\'zbek tillaridan birini tanlang. Tanlab bo\'lgach keyingisiga o\'ting.',
      'coach.language.voice': 'Til sozlamalarini bosib ko\'ring. Tanlab bo\'lgach keyingisiga o\'tish uchun bosing.',
      'coach.moreHelp.title': 'Bu yerda boshqa funksiyalarni ham ko\'rishingiz mumkin',
      'coach.moreHelp.desc': 'Pastdagi ma\'lumot·tarix·sozlamalarni bosib ko\'ring.',
      'coach.moreHelp.voice': 'Pastdagi menyuda boshqa funksiyalarni ham ko\'rishingiz mumkin.',
      'coach.next': 'Keyingisiga o\'tish',
      'coach.skipTutorial': 'Yo\'riqnomani o\'tkazib yuborish',
      'coach.sms1.title': 'Shubhali SMS-ni tekshirish',
      'coach.sms1.desc': 'Kelgan SMS xavfli yoki xavfsizligini tekshirib, xavfli bo\'lsa sababini ham aytib beramiz.',
      'coach.sms1.voice': 'Shubhali SMS-ni tekshirish — kelgan SMS xavfli yoki xavfsizligini bildiruvchi funksiya.',
      'coach.smsPermission.title': 'SMS o\'qishga ruxsat bering',
      'coach.smsPermission.desc': 'Ruxsat bersangiz, so\'nggi SMS-larni darhol ko\'rsatamiz.',
      'coach.smsPermission.voice': 'Ruxsat berish tugmasini bosing.',
      'coach.sms2.title': 'Tekshirish uchun shu SMS-ni bosing',
      'coach.sms2.desc': 'Bir marta bosish bilan darhol tekshirishingiz mumkin.',
      'coach.sms2.voice': 'Tekshirish uchun SMS-ni bosib ko\'ring.',
  }
};
