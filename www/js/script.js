...
async function createGuardianPairingCode(){
  const button = document.getElementById('createGuardianCodeButton');
  const panel = document.getElementById('guardianLinkCodePanel');
  const codeEl = document.getElementById('guardianLinkCode');
  const expiryEl = document.getElementById('guardianLinkCodeExpiry');
  if (!button || !panel || !codeEl || !expiryEl) return;
  if (!getAuth()) {
    showGlobalToast('로그인한 뒤 보호자 연결번호를 만들 수 있어요.');
    return;
  }
  button.disabled = true;
  button.textContent = '연결번호를 만들고 있어요…';
  try {
    const data = await guardianLinkRequest('/guardian-pair-code', { method: 'POST', body: '{}' });
    codeEl.textContent = String(data.code || '------').replace(/(\d{3})(\d{3})/, '$1 $2');
    const expires = new Date(data.expiresAt);
    expiryEl.textContent = Number.isNaN(expires.getTime())
      ? '10분 동안 사용할 수 있습니다.'
      : `${expires.getHours()}시 ${String(expires.getMinutes()).padStart(2, '0')}분까지 사용할 수 있습니다.`;
    panel.hidden = false;
    showGlobalToast('연결번호를 만들었어요. 보호자에게 알려주세요.');
  } catch (err) {
    showGlobalToast(err && err.status === 401 ? '다시 로그인해주세요.' : '연결번호를 만들지 못했어요. 잠시 후 다시 시도해주세요.');
  } finally {
    button.disabled = false;
    button.textContent = '새 연결번호 만들기';
  }
}
...
async function refreshGuardianLinkStatus(){
  ...
  const button = document.getElementById('createGuardianCodeButton');
  if (!statusEl || !listEl || !button) return;
  if (!getAuth()) {
    button.disabled = true;
    listEl.innerHTML = '';
